...
function resizeMasonry() {
  const grid = document.querySelector(".gp-masonry");
  if (!grid) return;
  ...
  grid.querySelectorAll(".gp-item").forEach(item => {
    const content = item.querySelector(".gp-media");
    if (!content) return;
...
function GaleriaVermas() {
  ...
  const [error, setError] = useState("");
    ...
    useEffect(() => {
      ...
      document.title = "Galeria | Vida Conectada";
    }, []);
    ...
    (async () => {
      ...
      try {
        ...
        const clean = fetched.filter(Boolean).map(it => ({
          ...
          title: it.title ?? "",
          caption: it.caption ?? "",
          media_url: it.image_url ?? ""
        }));

        console.log("RAW FETCHED:", fetched[0]);
            ...
            >
              {esVideo(it.media_url) ? (
                <video
                  ...
                  controls
                  preload="metadata"
                  onLoadedMetadata={resizeMasonry}
                />
              ) : it.media_url ? (
                <img
                  className="gp-media"
                  src={it.media_url}
                  alt={it.title || `Media ${idx + 1}`}
                  loading="lazy"
