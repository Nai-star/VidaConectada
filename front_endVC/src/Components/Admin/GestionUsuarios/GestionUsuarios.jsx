...
import ModalDetalleUsuario from "./ModalDetalleUsuario/ModalDetalleUsuario";
...
function parseParticipationDate(part) {
  ...
  for (const c of candidates) {
    ...
    // 3) Otros formatos: intentar Date directo (puede interpretar como UTC o local según navegador)
    const dfallback = new Date(str);
    if (!isNaN(dfallback.getTime())) return dfallback;
  }

  // intentar nested en metadata
...
async function confirmarEliminar() {
  ...
  try {
    await eliminarSuscrito(usuarioAEliminar.id);

    setSuscritos(prev =>
      prev.filter(u => String(u.id) !== String(usuarioAEliminar.id))
    );

    setSuscritosOriginales(prev =>
      prev.filter(u => String(u.id) !== String(usuarioAEliminar.id))
    );

    if (selected?.id === usuarioAEliminar.id) {
      setSelected(null);
    }
  } catch (error) {
...
function abrirModalEliminar(usuario) {
  ...
  return (
                  ...
                  return (
                    <tr key={key}>
                      ...
                      <td>
                        ...
                        <FaTint className="blood-icon" size={16} />
                      ...
                      <td className="actions-cell">
                        ...
                        {s.id && (
                        <button className="btn-delete" onClick={() => abrirModalEliminar(s)} title="Eliminar">
                            <FiTrash2 size={19} />
                        </button>
                        )}
                      </td>
    ...
    <ModalDetalleUsuario
      ...
      getBloodTypeBadgeClass={getBloodTypeBadgeClass}
      getDonorTypeBadgeClass={getDonorTypeBadgeClass}
      formatDateDisplay={formatDateDisplay}
      calcularUltimaFechaDeParticipaciones={calcularUltimaFechaDeParticipaciones}
    ...
    <AlertModal
      ...
      message={`¿Seguro que deseas eliminar a ${usuarioAEliminar?.nombre}? Esta acción no se puede deshacer.`}
